## 引入反思机制
# 内部反思和外部反思，外部反思精准率100%，就是召回率有点低（后续再改）
# 所以现在只考虑内部反思，这里重新跑一遍，并且加入反思提示词，并把结果保存在新的文件夹
...
import sys
sys.path.append('D:/FORGE2/Xblock/code/')  # 改成自己ML_Detection的相关路径
...
OPENAI_API_KEY = "XXXX"  # 替换为你的 API Key
...
if __name__ == "__main__":
    ...
    eventname = "bybit"
    '''
    source = "0x1542368a03ad1f03d96d51b414f4738961cf4443"
    is_malicious,label = LLM_Addr_Detect(source,eventname=eventname)
    print(is_malicious,label)
    '''
    #accounts_bfs(eventName=eventname, depth=0)
